...
|---|---|
| `merge_sql` (MERGE INTO) | ✅ [16 tests](../tests/io/test_merge_sql.py), no Spark: `on` and `actions` are the only route and both are required — each missing one refused by name, and the `on` error raised first when both are gone; every option of the old declarative form (`merge_keys`, `merge_condition`, `delete_when`, `delete_not_matched_by_source`) refused by name with the clause that replaces it, so a job written against the old syntax cannot run half-applied; and the clause list itself — whitespace normalised, a clause not starting with `WHEN` refused, an unconditional clause placed before another of its group refused as unreachable, and `NOT MATCHED BY SOURCE` not read as `NOT MATCHED` |
