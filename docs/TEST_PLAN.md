...
| `delta` | ✓ | ✓ | ✅ | [path-vs-table heuristic pinned](../tests/io/test_delta_path.py) and [run against a real jar](../tests/io/integration/test_lakehouse_spark.py): round trip by physical path, `append` adding to what was there, `mode: merge` updating one row and inserting another while leaving a third alone, the missing-`merge_keys` refusal, both DELETE branches (`delete_when` over a CDC file carrying an `op` flag, `delete_not_matched_by_source` over a full snapshot), `versionAsOf` still returning the pre-overwrite state, and `replaceWhere` swapping a single partition while the others stay put |
...
| `on_failure` modes | ✅ | 13 tests in [`tests/validation/test_on_failure.py`](../tests/validation/test_on_failure.py), no Spark: `fail` (the default when the key is absent) aborts with **nothing** written — not the destinations and not the report either, because the engine raises before the pipeline reaches it — and comes back as `PipelineResult(success=False)` with the failures in `error` rather than as an exception; `warn` and `skip` both continue and write report + destinations, with the failing results still on the result object; a `skip`ped validation does not set `PipelineResult.skipped`, which means something else entirely (`stop_if_empty`); and a result whose **severity** is `warn` never aborts, even under `fail`. Three of them go at `ValidationEngine.validate` directly, over a stubbed Cola, for the message the abort carries |
...
|---|---|
| `apply_template` (`params`) | ✅ [20 tests](../tests/utils/test_template_includes.py): every row of the formatting table (`str`/`int`, `True` → `"true"`, `False` → `""`, list of strings → `'a', 'b'`, list of numbers, empty list → `""`), the first item deciding the whole list, an unmatched key staying literal, the `\w+` key pattern, raw interpolation, and the collision with the runtime `{{var}}` — a param named after a runtime variable rewrites the reference before the engine sees it |
| `resolve_includes` (`$include`) | ✅ same file: single object, list expanded in order, `params` applied inside the included file, path relative to the main JSON, `FileNotFoundError` naming the path, and the two limits — not recursive, and only honoured inside `transformations` |
...
Two capabilities in this layer *are* pinned, both added in framework 0.7.0:

| Capability | Covered by |
|---|---|
| **`rows_written` read from the write itself** | [`tests/test_write_metrics.py`](../tests/test_write_metrics.py) — 17 tests. 11 need no Spark and cover the one thing that could corrupt a report, a history row or a bill: which accumulator is the write's own `number of output rows`. The scan publishes a metric of the same name, so the write's counter is identified by falling strictly between the anchor metrics its own tracker emits — pinned both ways (the write's counter is taken; the scan's never is). Also: the write runs exactly once, the listener bus is drained before reading, a non-write execution and a missing value answer `None` (*don't know*, never zero, which is what makes the `count()` fallback correct), and an unavailable or broken status store still leaves the data written. 6 run against real Spark, including a pipeline reading the same number both ways |
| **Execution history outside the Studio** | [`tests/test_observability_history.py`](../tests/test_observability_history.py) — 34 tests, no Spark. Off by default (no sink, no recorder, `Pipeline.run` unchanged); observing never breaks the run (a sink that raises, an HTTP refusal and a dead network are warnings, and the `PipelineResult` comes back unchanged); one run never collects another's records; the document's shape for success, failure and `skipped`, with its outputs, validations and identity; the ceiling that drops log lines but never step markers; `SPARQUET_HISTORY_LOGS=off`; the HTTP sink's method, body, token header and timeout; and the environment read once. The runner's half is 16 more in `server/test_history.py` |
...
| Execution history | ✅ | server: 29 `unittest` tests over `ExecutionRepository`/`StepTracker`/`lineage_of` (success, mid-step failure, skip propagation, listing/ordering and its `job_id` filter across nested job runs, FK-enforced no-orphan-`StepRun`, the run log: arrival order, one `seq` sequence across batches, no leaking between job runs; and the run-detail fields: `run_as`/`launched` recorded and defaulted to `manual`, `lineage` stored and read back whole, the migration that adds all three to a DB written before they existed) · 7 of them on `lineage_of` itself, which derives inputs/outputs from the submitted config: join as a second input, the three quality sinks kept apart by role, list-valued inputs/outputs, the Kafka topic inside `options`, the JDBC `dbtable`, and anything unrecognisable read as no lineage; client: 23 tests over `lib/runner/history.ts`'s mapping, the 404→`null` contract, the `cancelled` status, `getJobRunLogs`'s paging by `seq`, the lineage JSON string parsed (and an unparseable one read as `null`), and `run_as`/`launched`/`lineage` all left `null` on a run recorded before they existed |
| History retention | ✅ | 16 `unittest` tests over the two-stage purge: a recent run untouched; an old one losing logs, steps and stored JSON while keeping its row, its counts and its `config_hash`; thinning alone never deleting a row (`delete` is off by default); the second stage deleting only with `delete` on; a `pinned` run surviving both stages and coming back into reach when unpinned; the newest `keep_runs` executions kept however old, counted **per Job** so a busy Job cannot push a quiet one's history out; a `dry_run` counting exactly what the real pass then removes; an empty history as a no-op; and `VACUUM` firing only past the threshold. Plus 4 on `RetentionPolicy`: the documented defaults, each number overridden from the environment, a value that does not parse falling back instead of stopping the runner, and the purge turned off entirely |
...
| Billing tags | ✅ | 12 `unittest` tests on the catalog half (`test_history.py`): tags kept on a Workflow, a Pipeline and a Job; a re-save **replacing** them rather than merging; `tags=None` meaning "not mentioned" so a rename cannot silently untag while `tags=[]` clears them; a Job inheriting its Workflow's and its Pipeline's; the same tag written `Prod` and `prod` counted once across levels; the ceiling and the length cap; an unknown kind raising; and `list_tags` counting the records that carry each one · 12 more on the billing half (`test_credits.py`): `group_by=tag` grouping, the untagged row, a month with none, the tags **frozen on the entry** so retagging a Job does not rewrite a closed invoice, the overlap — a run with two tags counted in full under each, so the rows sum to more than `totals()`, which counts every entry once — the account scope, `settle` carrying them through a reservation, and grants staying out of it · 6 on `totals()` and `usage_timeline()`: a quiet month present as zeros rather than missing, the oldest-first order, and `months` clamped · client: 11 tests on `lib/tags.ts` mirroring the server's normalisation (trim, dedupe case-insensitively keeping the typed case, both caps, the same array identity back when nothing changed) and 11 on `lib/billing.ts` (the period is UTC on both sides, the six months walked back, a bar never drawn past its track, and no invented percentage when the month before was zero) |
| Audit log | ✅ | server: 18 `unittest` tests on `server/audit.py` — every state-changing request recorded with who, what, on which resource and with which outcome; a refused request recorded **with** its `denied` outcome and whatever identity it had, including none; the filters (`actor_id`, `resource`, `outcome`, `action` by `iam:*` prefix, `since`) and their ordering; bodies never stored, only the named fields; and `iam:ReadAudit` required to read it back · client: 7 tests on `lib/runner/audit.ts` — the mapping, the filters in the query string (`action=iam%3A*`), an anonymous denied event rendered rather than crashed on, a missing outcome defaulted to `allowed`, a non-array payload read as no events, and `isForbidden` separating "you may not read this" from every other failure |
...
| AI assistant | ◐ | 28 tests over the streaming client and the parser — the transport and the parse of a proposal into a graph. What is **not** pinned is whether the proposal is any good: that `lib/ai/prompt.ts` really carries the catalog (a transformation, format or validator missing from `src/catalog/` silently disappears from the prompt and the model starts inventing a `type` that does not exist), that an accepted proposal survives `compileGraph` and passes the linter, that each provider in `lib/ai/providers.ts` still speaks its own endpoint/auth/event format, and that a bad answer (JSON truncated mid-stream, prose outside the block, an unknown `type`, a refused API key) reaches the user as a message rather than as an empty canvas |
