...
| Studio | — | 452 tests + 19 smoke checks |
| Runner service (`server/`) | — | 131 `unittest` tests (history 48, auth 44, credits 29, workspace 10) |
...
The Studio is the best-covered part of the project (452 unit tests, 19 smoke checks in
...
| Runner service (`server/main.py`) | ⬜ | still **no test** for the request/response layer itself: token auth, the origin allow-list, the `403` on `/run`, the SSE event sequence, and the `requires(...)` dependency that maps each route to an action (the evaluator behind it is pinned — see *Identity and permissions* — but no test asserts that `PUT /workspace` is the route demanding `workspace:Write`). The execution-history side-effects of these endpoints (`server/history.py`) are covered — see the row above — but the endpoints themselves are only ever exercised by hand |
