...
| Studio | — | 491 tests + 21 smoke checks |
| Runner service (`server/`) | — | 289 `unittest` tests (history 92, auth 64, credits 89, audit 18, run scope 16, workspace 10) |
...
The Studio is the best-covered part of the project (491 unit tests, 21 smoke checks in
a real Chrome, and 289 `unittest` tests on the runner: 92 on the history database, 64 on
identity, 89 on credits, 18 on the audit log, 16 on the scope of the execution routes and
10 on the workspace). What is pinned, and what is not:
