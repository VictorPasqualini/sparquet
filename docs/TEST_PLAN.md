...
| Studio | — | 469 tests + 21 smoke checks |
| Runner service (`server/`) | — | 259 `unittest` tests (history 80, auth 64, credits 71, audit 18, run scope 16, workspace 10) |
...
The Studio is the best-covered part of the project (469 unit tests, 21 smoke checks in
a real Chrome, and 259 `unittest` tests on the runner: 80 on the history database, 64 on
identity, 71 on credits, 18 on the audit log, 16 on the scope of the execution routes and
