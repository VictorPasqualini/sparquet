...
Esta página parte de uma máquina sem nada instalado e termina com um pipeline que você construiu no canvas, executado contra o Spark real, exibindo seu resultado. Ela amarra as páginas de [instalação](/pt/docs/start/install/) e de [execução de pipelines](/pt/docs/studio/running/) num único caminho ordenado — siga-o de cima a baixo na primeira vez.
...
3. Abra [http://localhost:5273](http://localhost:5273). O primeiro lançamento semeia um projeto **Getting Started** com pipelines funcionais.
...
- O fluxo completo de execução, endpoints e solução de problemas: [Executando pipelines](/pt/docs/studio/running/).
