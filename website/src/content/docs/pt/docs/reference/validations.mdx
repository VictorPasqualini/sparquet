...
<Aside type="tip">
  `validations.outputs` é o split "bronze → silver válido + silver inválido" quando
  *inválido significa "falhou nas regras de DQ"*. Se o split for lógica de negócio não relacionada à
  qualidade, faça-o com duas entradas nos `outputs` do pipeline e um `filter` por
  destino em vez disso. Veja o guia de qualidade de dados para o trade-off.
</Aside>
