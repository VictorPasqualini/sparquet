---
title: Executando um Job
description: Execute um Job pelo canvas do Sparquet Studio via runner local — setup, o token de segurança e leitura dos resultados.
sidebar:
  order: 5
---
...
<Aside type="danger" title="Por que existe um token">
  ...
  que você visite poderia postar um JSON de pipeline em `127.0.0.1:8787` e rodá-lo; o CORS
...
Para rodar **vários Jobs um depois do outro**, monte um [Pipeline](/pt/docs/studio/pipelines/) — mesmo runner, mesmo token, um estágio por Job.
...
| `POST /run` | token | roda um JSON de pipeline, retorna contadores, validações, preview e logs |
| `POST /run/flow/stream` | token | roda vários JSONs em sequência — o que um [Pipeline](/pt/docs/studio/pipelines/) posta |
