...
```json
{
  ...
  "input": {                           // required
    "format": "csv|parquet|delta|iceberg|txt|view|postgres|mysql|sqlserver|oracle|jdbc",
    "path": "string",
    "options": {}
  },
...
| Key | Default | Notes |
|---|---|---|
| `app_name` | `SparkFramework` | Overridden by the `SparkFramework(spark=…)` constructor |
...
```json
{
  ...
  "description": "Daily revenue per customer, curated and published back to the operational database.",
