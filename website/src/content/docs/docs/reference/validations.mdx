...
<Aside type="tip">
  `validations.outputs` is the "bronze → valid silver + invalid silver" split when
  *invalid means "failed the DQ rules"*. If the split is business logic unrelated to
  quality, do it with two entries in the pipeline `outputs` and a `filter` per
  destination instead. See the data-quality guide for the trade-off.
</Aside>
