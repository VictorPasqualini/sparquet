...
3. ### Run it
   ...
   fw = SparkFramework(spark={"app_name": "demo", "master": "local[*]"})
   ...
   ```bash
   python -m spark_framework.cli orders.json
   ```
...
- [Your first flow in Studio](/docs/start/first-flow/) — the same pipeline, drawn on a canvas.
