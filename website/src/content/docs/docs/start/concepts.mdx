...
Studio does not store a proprietary project format — it compiles the graph. Two rules define the mapping:

**1. The shared chain is the main pipeline.** The transformations every destination has in common become the top-level `transformations`. Once the graph forks, each branch becomes that destination's own `transformations`.

```
source → filter → cast ─┬─→ [group_by] → postgres     ← group_by belongs to this output
...
The session is a process-wide singleton: the first `SparkFramework` instance wins, and later pipelines share it. That is what makes running several pipelines in one job cheap.
...
- [Guides](/docs/guides/databases/) — end-to-end recipes.
