---
title: Running a job
description: Execute a Job from the Sparquet Studio canvas through the local runner — setup, the security token, and reading the results.
sidebar:
  order: 5
---
...
<Aside type="danger" title="Why there is a token at all">
  ...
  you happen to visit could post a pipeline JSON to `127.0.0.1:8787` and run it;
...
To run **several Jobs one after another**, build a [Pipeline](/docs/studio/pipelines/) instead — same runner, same token, one stage per Job.
...
| `POST /run/flow/stream` | token | run several JSONs in sequence — what a [Pipeline](/docs/studio/pipelines/) posts |
