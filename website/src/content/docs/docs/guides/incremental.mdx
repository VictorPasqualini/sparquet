...
The same mode works for the [database connectors](/docs/reference/connectors/#write-modes), where it runs the vendor's own upsert.

<Aside type="caution" title="One row per key">
  A merge fails when the incoming DataFrame has more than one row per key —
  Delta raises *multiple source rows matched*, and a database silently inserts
  twice. Deduplicate before writing, and add a `unique` validation on the merge
  keys so the pipeline reports it rather than the storage layer.
</Aside>
...
```json
{ "options": { "query": "SELECT * FROM orders WHERE updated_at > '{since}'" } }
```
