---
title: Ejecutar pipelines
description: Ejecuta un pipeline desde el canvas de Sparquet Studio mediante el runner local — setup, el token de seguridad y lectura de resultados.
sidebar:
  order: 5
---
...
<Aside type="danger" title="Por qué hay un token">
  ...
  que visites podría postear un pipeline a `127.0.0.1:8787` y ejecutarlo; CORS no
...
- **Run pipeline** compila el canvas y lo ejecuta.
...
Cuando el workflow declara placeholders `{param}`, el panel renderiza un input por parámetro y envía los valores con la ejecución.
...
| `POST /run` | token | corre un pipeline, devuelve contadores, validaciones, preview y logs |
