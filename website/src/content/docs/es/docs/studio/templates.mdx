...
description: Diez pipelines funcionales y seis lecciones guiadas que vienen con Sparquet Studio.
sidebar:
  order: 6
---

Studio viene con pipelines que realmente corren. Abrir uno y leer su JSON suele ser más rápido que leer una página de referencia.

## Plantillas

**Templates** en la barra lateral. Cada tarjeta trae lo que el pipeline demuestra; **Preview** muestra el JSON antes de comprometerte con nada, **Use template** lo copia a un proyecto como un nuevo workflow.
...
| Staging view handoff | intermediate | Varios pipelines componiendo por una temp view |
...
1. **Tu primer pipeline en cinco minutos** — la forma del documento y la ejecución
...
El camino más rápido a un nuevo pipeline rara vez es un canvas en blanco:

1. Encuentra la plantilla más cercana a tu caso.
2. **Use template** en el proyecto correcto.
