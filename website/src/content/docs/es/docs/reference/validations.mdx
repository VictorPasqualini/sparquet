...
<Aside type="caution" title="Es una copia, no una bifurcación">
  `validations.outputs` **no** desvía filas. El pipeline escribe estos destinos y
  luego escribe su propio `output`/`outputs` desde el **mismo DataFrame completo** —
  incluidas las inválidas. Un pipeline con cuarentena escribe cada fila mala dos veces.

  Es decir, **audita**: qué filas rompieron una regla, apartadas para inspección, sin
  cambiar el flujo.

  Para **enrutar** — que solo las filas buenas lleguen a la tabla final — usa el
  pipeline: un `filter` antes de cada destino. Reescribes la condición en SQL
  (mantenla al día con las reglas) y, a cambio, el destino principal lleva exactamente
  lo que querías.
</Aside>
