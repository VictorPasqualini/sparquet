...
Studio lista los parámetros de un workflow y renderiza un input para cada uno — tipado como texto, número, switch o chips — y los envía con una ejecución local. También detecta con el linter los dos desajustes que muerden: un `{param}` usado pero nunca declarado, y un parámetro declarado que nada usa.
