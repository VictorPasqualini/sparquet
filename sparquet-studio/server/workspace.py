...
@dataclass
class LibraryFile:
    """One runnable JSON in the library, as the interface lists it.

    A path here is always **relative to the library root** and uses forward
    slashes. Never an absolute one: a Pipeline that named `C:/Users/ana/...`
    would stop working the moment it was opened on another machine, and it would
    do it silently.
    """
...
def _check_relative(relative: str) -> str:
    """A library path comes from the client, so it is checked, not trusted.

    Relative, forward slashes, JSON, and nothing that could climb out of the root
    — `_resolve` refuses an escape as well, and both checks stay: this one gives
    the reason, that one is the last line.
    """
    value = str(relative or "").strip().replace("\\", "/").lstrip("/")
    if not value:
        raise WorkspaceError("Give the path of a file in the library.")
    if not value.lower().endswith(".json"):
        raise WorkspaceError(f"Not a pipeline file: {relative!r}. Expected a .json.")
    parts = [part for part in value.split("/") if part]
    if any(part == ".." for part in parts):
        raise WorkspaceError(f"Path escapes the workspace: {relative!r}.")
    if parts and parts[0] == _STUDIO_DIR:
        raise WorkspaceError(
            f"{_STUDIO_DIR}/ is the editor's own state, not something to run."
        )
    if Path(value).is_absolute() or (len(value) > 1 and value[1] == ":"):
        raise WorkspaceError(
            f"Give a path relative to the library root, not {relative!r}: an "
            "absolute path stops working on the next machine."
        )
    return "/".join(parts)
...
class FileWorkspaceStore:
    ...
    def list_files(self) -> List[LibraryFile]:
        ...
        This is the whole tree, not only what the Studio wrote: a file dropped in
        by another team, generated by a script or written by hand is exactly the
        case this exists for. `.studio/` is excluded — that is the editor's own
    ...
    def read_file(self, relative: str) -> Dict[str, Any]:
        ...
        Read at the moment it is needed and never cached: the point of pointing a
        stage at a file is that the file is the source of truth, so an edit made
        outside the Studio takes effect on the next run without anything being
        re-imported.
        """
        candidate = self._resolve(_check_relative(relative))
        if not candidate.is_file():
            raise WorkspaceError(f"No such file in the library: {relative!r}.")
        try:
            payload = json.loads(candidate.read_text(encoding="utf-8"))
        except OSError as error:
            raise WorkspaceError(f"{relative!r} cannot be read: {error}") from error
        except ValueError as error:
            raise WorkspaceError(f"{relative!r} is not valid JSON: {error}") from error
        if not isinstance(payload, dict):
            raise WorkspaceError(
