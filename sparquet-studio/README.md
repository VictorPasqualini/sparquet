...
The three things that block **writes** are boxes too — three more entries in the
same **Quality** section of the palette, added by a click or a drag like any other
node:
...
> every destination on the main chain — which is exactly why these boxes sit off the
> chain rather than on it: nothing is taken off it. If the main destination must not
> carry the bad rows, remove them yourself with a `filter`.
...
> **Spark is lazy, so a green transformation box means the step *compiled*, not
> that data went through it.** Nothing is executed to produce that status: the
> box turns green when the framework has added the operation to the plan, and
> expect the transformation boxes to flick past almost instantly for exactly that
> reason. A transformation that is wrong in a way only the executor discovers —
> a cast that overflows, an expression that fails on a value that has not been
> read yet — is marked succeeded here, and the error surfaces on the next step
> that forces an action: a validation, or a write. The wall-clock time belongs to
> those steps too, since they are the ones that touch data.
>
> This is a deliberate trade. Making a transformation box mean "the data passed"
> would mean forcing a `count()` after every one, which reads the whole input
> again per step — the exact cost lazy evaluation exists to avoid. The steps that
> do report on data are the ones that were going to touch it anyway: the read
> (`input`), the validations and the writes.
...
2. **Add stages.** The Workflow's Jobs are listed on the left — click one to append it at the end of the row, or drag it onto the canvas. The same Job may appear more than once. Below them, **Files in the library** lists every `.json` in the library directory: a stage can point at a file instead of a Job (see below).
