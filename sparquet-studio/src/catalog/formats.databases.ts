...
const jdbcReadOptions = (driver: string, port: string): FieldSpec[] => [
  ...jdbcConnectionFields(driver, port),
  {
    ...
    help: 'Reads the result of this SQL instead of the table in the path. Mutually exclusive with dbtable and with partitionColumn.',
    docs: [
      'Filtering inside the database is the cheapest read there is: what never leaves the server costs no',
      'network and no executor memory.',
      '',
      'Spark refuses `query` together with `dbtable`, and also together with `partitionColumn`. To read a',
      'narrowed table in parallel, move the SELECT into `dbtable` as a subquery **with an alias** —',
      '`(SELECT id, valor FROM vendas WHERE dt >= DATE \'2026-09-01\') t` — and keep the partitioning',
      'quartet. The reader refuses both combinations before any Spark call.',
    ].join('\n'),
    group: 'advanced',
  },
  {
    key: 'dbtable',
    label: 'Table or subquery',
    type: 'text',
    placeholder: '(SELECT id, valor FROM vendas WHERE dt >= DATE \'2026-09-01\') t',
    help: 'Overrides the path. A subquery is allowed here and MUST carry an alias.',
    docs: [
      'This is the only form that combines a filter pushed into the database with a parallel read, since',
      '`query` and `partitionColumn` cannot coexist. Spark wraps the value in `SELECT * FROM <value>`, so a',
      'subquery without a trailing alias is a SQL syntax error on the server.',
    ].join('\n'),
    group: 'advanced',
  },
  {
    ...
    docs: [
      'Without it the whole table arrives through **one connection in one task**, no matter how many',
      'executors the cluster has. With it, Spark issues `numPartitions` queries, each with its own',
      '`WHERE col >= a AND col < b`.',
      '',
      'The column has to be numeric, date or timestamp — **and indexed**. These are N range queries: with',
      'no index they are N full scans, and reading in parallel ends up slower than not.',
    ].join('\n'),
    group: 'advanced',
  },
  { key: 'numPartitions', label: 'Num partitions', type: 'number', placeholder: '8', group: 'advanced' },
  {
    ...
    label: 'Lower bound',
    type: 'text',
    help: 'Start of the split ruler — NOT a filter.',
    docs: [
      '`lowerBound` and `upperBound` only decide where the partition boundaries fall. They filter nothing:',
      'rows outside the range still come, packed into the two edge partitions, which then run fat. Use',
      '`query` or a `dbtable` subquery to actually filter.',
      '',
      'Take them from a `SELECT min(col), max(col)` — a stale upper bound is the usual cause of one',
      'partition holding most of the table.',
    ].join('\n'),
    group: 'advanced',
  },
  { key: 'upperBound', label: 'Upper bound', type: 'text', help: 'End of the split ruler — NOT a filter.', group: 'advanced' },
  {
    key: 'fetchsize',
    label: 'Fetch size',
    type: 'number',
    placeholder: '1000',
    help: 'Rows per network round-trip. The driver defaults are rarely right.',
    docs: [
      'The JDBC driver default is almost never a good value: PostgreSQL streams the whole result set at once',
      '(a real OOM risk on the executor), while Oracle fetches 10 rows at a time (one round-trip per 10',
      'rows). Something in the 1,000-10,000 range is the usual compromise.',
    ].join('\n'),
    group: 'advanced',
  },
  {
    key: 'pushDownPredicate',
    label: 'Push down predicate',
    type: 'select',
    options: BOOL_OPTIONS,
    help: 'Default true — sends WHERE clauses to the database instead of filtering in Spark.',
    docs: [
      'On by default, so a `filter` placed first in the transformation chain already becomes part of the',
      'SQL sent to the server. Set it to `false` only to move the work off a database that is the',
      'bottleneck.',
    ].join('\n'),
    group: 'advanced',
  },
  {
    key: 'pushDownAggregate',
    label: 'Push down aggregate',
    type: 'select',
    options: BOOL_OPTIONS,
    help: 'Sends group_by aggregations to the database (Spark 3.2+).',
    group: 'advanced',
  },
  {
    key: 'pushDownLimit',
    label: 'Push down limit',
    type: 'select',
    options: BOOL_OPTIONS,
    help: 'Sends LIMIT to the database (Spark 3.3+).',
    group: 'advanced',
  },
  {
    ...
    help: 'SQL run on every partition connection before the read query.',
    group: 'advanced',
  },
  { key: 'queryTimeout', label: 'Query timeout (s)', type: 'number', placeholder: '0', group: 'advanced' },
]
...
const jdbcFormat = (opts: {
  ...
  gotchas: [
    ...
    'partitionColumn/lowerBound/upperBound/numPartitions are all-or-none. partitionColumn without the other three raises a ValueError naming what is missing; the reverse — bounds with no partitionColumn — Spark ignores silently and reads in a single task, so the framework only warns.',
