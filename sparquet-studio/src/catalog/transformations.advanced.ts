...
const includeDef: TransformationDef = {
  ...
  description: lines(
    ...
    '',
    '`{param}` placeholders inside the included file are substituted with the same values as the main file — but only when the pipeline is run through `SparkFramework.run` / `run_from_dict`. Loading a config directly through `Pipeline.from_file` expands the include and leaves the placeholders literal.',
    '',
  ...
  gotchas: [
    ...
    '`{param}` substitution inside the included file only happens through `SparkFramework.run` / `run_from_dict`. `Pipeline.from_file` expands the include but leaves placeholders literal.',
