...
export const TEMPLATES: WorkflowTemplate[] = [
  ...
  {
    id: 'database-to-lakehouse',
    name: 'Database to lakehouse',
    summary:
      'Read a production table from PostgreSQL in parallel, land it as Delta, and upsert the aggregate back into the database.',
    highlights: [
      'partition_column plus lower_bound, upper_bound and num_partitions open N connections; without them one executor reads the whole table',
      'password_env names an environment variable, so the committed pipeline carries no credential',
      'A database destination with mode "merge" writes a staging table and runs the vendor upsert (ON CONFLICT here)',
    ],
    level: 'intermediate',
    tags: ['postgres', 'jdbc', 'ingestion', 'merge'],
    pipeline: {
      name: 'database_to_lakehouse',
      description:
        'Parallel read from PostgreSQL, curated Delta table, and the daily aggregate upserted back into the source database.',
      spark: {
          ...
          transformations: [
            {
              type: 'group_by',
              by: ['customer_id'],
              agg: ['sum(amount) as revenue', 'count(*) as orders', 'max(order_date) as last_order'],
            },
          ],
          options: {
            host: 'db.internal',
