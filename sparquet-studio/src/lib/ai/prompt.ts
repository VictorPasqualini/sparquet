...
const HARD_RULES = [
  ...
  '{secret:name/field} is a connection credential — a password, a JDBC URL, a token. Write the reference, never the value: the runner replaces it on the way to Spark and the Job JSON keeps only the reference. Prefer it to a literal password in any option, and never invent a secret name that the user has not mentioned.',
