...
(NC, CPR, Duplicata, CCB) para confs declarativas do sparquet. Acompanha o que já
...
> quality/governança, etc.), ver [BACKLOG.md](../../BACKLOG.md).
...
Movido para o roadmap do framework → [BACKLOG.md](../../BACKLOG.md) (dry-run, testes
...
A migração de um conjunto real de **jobs Spark de registro de lastros** (Nota
Comercial/B3, CPR/B3, Duplicata VERT, Duplicata CERC e — pendente — CCB/CERC), antes
escritos como scripts PySpark imperativos (`tests/case-of-success/old/`), para
**confs declarativas do sparquet**. O objetivo foi provar, num caso de produção com
payloads aninhados, múltiplos fluxos e verificação de qualidade, que **o Python só
orquestra** e a lógica de dados vive na configuração.

### O que resultou de aprendizado

1. **O caso guiou o design do núcleo.** As capacidades genéricas do framework
   (`distinct`, `checkpoint`, `collect` + `{{var}}`, `struct`, `with_column` múltiplo,
   `transformations` por output, `stop_if_empty`, warnings adiados) nasceram de
   necessidades reais desta migração — mas entraram **sem acoplamento de domínio**,
   servindo a qualquer pipeline. Migrar um job de verdade é o melhor filtro de design.

2. **Padrão staging → commit.** Confs "produtoras" só montam o payload e gravam num
   staging genérico; uma conf de **commit** única lê o staging, **verifica** (via
   `validations`) e escreve os destinos. Eliminou a duplicação dos 3 outputs em cada
   conf e centralizou a verificação — reutilizável para qualquer fan-in de múltiplas
   fontes.

3. **Branch por parâmetro numa conf só.** `skip_if_false` com **expressão booleana**
   (ex.: `"'{fluxo_operacao}' in ('EMISSAO', 'EMISSAO_E_REGISTRO')"`) permitiu os 3
   fluxos da Duplicata/CERC num único arquivo, com o orquestrador passando apenas o
   parâmetro do fluxo.

4. **Pushdown declarativo.** `collect` + `{{var}}` substituiu o `df.collect()` +
   `col.isin(...)` imperativo, empurrando o filtro `IN (...)` para as leituras
   seguintes (predicate pushdown / data skipping) sem sair da conf.

5. **A fronteira "config vs código" ficou empírica.** Duplicata (VERT e CERC) coube
   em **conf pura** e legível. O CCB — com parametrização pesada, resolução de coluna
   e construção de IPOC — é o caso que provavelmente pede o **híbrido** (payload como
   `register_transformation`, testável em Python). O critério não é teórico: é onde o
...
Falta migrar o **CCB/CERC** (decisão conf pura × híbrido — ver §3.1), o fluxo mais
pesado. Ele é justamente o teste-limite da tese do framework: se couber de forma
legível, a fronteira do declarativo é ampla; se pedir transformação registrada, o
ponto de extensão `register_transformation` é a resposta prevista.
