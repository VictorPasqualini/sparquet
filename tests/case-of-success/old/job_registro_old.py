...
for (tipo_ativo, registradora), fluxo_operacao in fluxos_operacoes_processamento.items():
    
    tipo_contrato = CODIGOS_TIPO_CONTRATO[tipo_ativo]

    # Itera sobre os tipos de fluxo dentro do fluxo de operação (REGISTRO, EMISSAO ou EMISSAO_E_REGISTRO)
    for tipo_fluxo, atributos_fluxo in fluxo_operacao.items():
        
        df_registro = df_cessoes_pendentes_de_registro.filter(
            (col("tipo_ativo") == tipo_ativo)
            & (col("registradora") == registradora)
            & (col("fluxo_operacao") == tipo_fluxo)
            & (
                (
                    (col("tipo_contrato") == tipo_contrato) 
                    & (col("multi_ativos") == True)
                ) 
                | (col("multi_ativos") == False)
            )
        )

        df_cessoes_nao_enviadas_para_registro = df_cessoes_nao_enviadas_para_registro.join(
        ...
        if df_registro.isEmpty():
            print(f"ℹ️ NÃO EXISTEM CESSOES PENDENTES PARA - {tipo_ativo}-{registradora} ({tipo_fluxo})")
            ...
            try:
                ...
                retorno_estrutura = atributos_fluxo["funcao_construcao_payload"](obj=objeto_preparacao)
            ...
            # Captura tópico kafka correspondente ao tipo de fluxo
            df_envio_registro, df_registro_estrutura = retorno_estrutura
            topico_kafka = atributos_fluxo["topico"]

            df_envio_registro.display()
            df_registro_estrutura.display()

            lista_cessoes_processadas = [
                r.id_cessao
                for r in df_registro.select("id_cessao").distinct().collect()
            ]

            print(f"ℹ️ CESSOES PROCESSADAS {lista_cessoes_processadas} PARA - {tipo_ativo}-{registradora} ({tipo_fluxo})")

            try:
                # Envia mensagem ao tópico kafka
                (
                    df_envio_registro
                    .write
                    .format("kafka")
                ...
                # Atualiza tabela 'silver_controle_registro_cessoes'
                (
                    df_registro
                    .select("id_cessao", "id_operacao")
                    .distinct()
                ...
                # Atualiza tabela 'silver_registro_contratos'
                (
                ...
                # Atualiza tabela 'silver_registro_parcelas'
                (
                    df_registro_estrutura
                ...
                detalhes_erro = traceback.format_exc()
                print(f"❌ ERRO NA EXECUÇÃO DO KAFKA/PERSISTÊNCIA - {tipo_ativo}-{registradora} ({tipo_fluxo}): \n{detalhes_erro}")
                continue

            df_registro_estrutura.unpersist()
            df_envio_registro.unpersist()

if df_cessoes_nao_enviadas_para_registro.isEmpty():
    print("ℹ️ TODAS AS CESSÕES FORAM PROCESSADAS!")

else:
    print("ℹ️❌ CESSÕES QUE NÃO FORAM PROCESSADAS, MOTIVO MAIS PROVÁVEL: CAMPO tipo_contrato VEIO ERRADO PARA O tipo_ativo ESPECIFICADO")
    df_cessoes_nao_enviadas_para_registro.select("id_cessao").dropDuplicates(["id_cessao"]).display()

df_cessoes_aprovadas_registro.unpersist()
df_cessoes_aprovadas_parcialmente.unpersist()
df_cessoes_pendentes_de_registro.unpersist()
